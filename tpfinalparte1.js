...
let pantallas = [];
let click; // Asegúrate de que 'click' está declarado para el sonido
let archivos = [
    ...
    "Pantalla_12.jpg","Pantalla_13_FinalOriginal.jpg", "Pantalla_14_FinalBueno.jpg","Pantalla_15_FinalMalo.jpeg", "Pantalla_16.jpeg"];
...
function manejarClickBotones() {
    ...
    for (let i = 0; i < total; i++) {
        let x = startX + i * (anchoBoton + espaciado);
        if (
            mouseX > x && mouseX < x + anchoBoton && mouseY > y && mouseY < y + altoBoton
        ) {
